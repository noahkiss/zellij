...
pending-wrap flag as a side effect (mobile-shell/mosh#687). Zellij emits an OSC 8 link *end* at the
start of essentially every rendered row, so over mosh that flag was being cleared constantly, and
the cell in the right-most column was lost each time. The visible result is a column of missing
characters down the right edge of the screen. BEL takes a different path in that emulator and leaves
the flag alone.
...
own window-title OSC with BEL. The practical effect is that `osc8_hyperlinks true`, which is the
default, stays usable over mosh instead of having to be turned off.
